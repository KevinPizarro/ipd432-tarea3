...
//----------------------------------------  DATA OUT -------------------------------------    
    ...
    save_and_transmit #(WIDTH,DEEPTH) mem_tx_control(
        .clk_main(CLK100MHZ),.clk_logic(CLK100MHZ),.rst(~CPU_RESETN),
...
*/
    logic [WIDTH-1:0] [DEEPTH-1:0] vectA, vectB;
    logic [WIDTH-1:0] [DEEPTH-1:0] vectSum, vectAvg, vectMan;
    logic [WIDTH-1:0] [DEEPTH-1:0] sum_adder, avg_adder;
    logic enable_data;
    
    data_enable #(WIDTH, DEEPTH) dataA(
        .en(enable_data),.data_in(memA),.data_out(vectA)
    );
    data_enable #(WIDTH, DEEPTH) dataB(
        .en(enable_data),.data_in(memB),.data_out(vectB)
    );
        
// ------------------------------- OERATIONS ---------------------------------        
    n_bit_multiadder #(WIDTH, DEEPTH) adder(        // modulo 
        .sum_adder(sum_adder),.avg_adder(avg_adder),
        .A(vectA),.B(vectB)
    );
    
    SumVecOperation #(WIDTH, DEEPTH) Sum_Vect_inst(
        .clk(CLK100MHZ),.sum_adder(sum_adder),.sum(vectSum)
    );
    
    AvgVecOperation #(WIDTH, DEEPTH) Avg_Vect_inst(
        .clk(CLK100MHZ),.avg_adder(avg_adder),.avg(vectAvg)
    );
    
    ManDisOperation #(WIDTH, DEEPTH) Man_Dist_inst(
        .clk(CLK100MHZ),.A(vectA),.B(vectB),
