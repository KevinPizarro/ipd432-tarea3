...
/********************************** CLOCK MAGNEMENT ************************************/
      clk_wiz_0 clkwiz
    (
        .CLKLOGIC(CLKLOGIC),    // output CLKLOGIC
...
//  ADDR ITERATION    
    ...
    counter_with_inc #(DIRW) master_count(
        .clk(CLKLOGIC),.rst(~CPU_RESETN || job_ok),
...
//----------------------------------------  DATA OUT -----------------------------------
    ...
    save_and_transmit #(WIDTH,DEEPTH) mem_out_control(
        .clk(CLKLOGIC),.rst(~CPU_RESETN),
        .en_op(en_data),.tx_busy(tx_busy),.addr_inc(tx_inc),
...
/************************************** ILA *******************************************/    
//ila_0 ILA (
//	.clk(CLKLOGIC),          // input wire clk 
//	.probe0(rx_ready),       // input wire [0:0]  probe0  
//	.probe1(en_data),        // input wire [0:0]  probe1  
//	.probe2(writeOut[0]),    // input wire [0:0]  probe2  
